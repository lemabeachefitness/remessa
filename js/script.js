...
const products = [
    ...
    { name: 'Short League', icon: 'fas fa-male' },
...
function exportToPDF() {
    ...
    for (const [product, items] of Object.entries(groupedLaunches)) {
        ...
        const groupedByDetails = items.reduce((acc, item) => {
            const key = `${item.gender} - ${item.size} - R$ ${item.unitPrice.toFixed(2)}`;
            if (!acc[key]) {
                acc[key] = 0;
            }
        ...
        for (const [details, quantity] of Object.entries(groupedByDetails)) {
            doc.text(`  - ${quantity} ${details}`, 20, yPos);
            yPos += 7;
        }
